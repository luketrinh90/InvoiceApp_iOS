//
//  ReceiptSubCategoryViewController.swift
//  InvoiceApp
//
...
class ReceiptSubCategoryViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    var array: [String]?
    var passedInvoiceObject: Invoice?
    var passedReceiptObject: Receipt?
    var defaultOverStayObject: OverStay?
    
    var viewModel: ReceiptSubCategoryViewModel?
    let realm = try! Realm()
    ...
    func initFirst() {
        viewModel = ReceiptSubCategoryViewModel()
    ...
    func readPropertyList() {
        ...
        let plistPath:String? = NSBundle.mainBundle().pathForResource(NSLocalizedString((passedReceiptObject?.category)!,comment:""), ofType: "plist")! //the path of the data
        ...
        do { //convert the data to a dictionary and handle errors.
            ...
            array = plistData[NSLocalizedString((passedReceiptObject?.category)!,comment:"")] as? [String]
        }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCellWithIdentifier("receiptSubCategoryTableViewCell") as! ReceiptSubCategoryTableViewCell
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if let arr = array {
            if passedReceiptObject?.category == "Service" {
                if let vc = self.storyboard?.instantiateViewControllerWithIdentifier(NotificationConstants.ViewController.SubServiceViewController) as? SubServiceViewController,
                    ...
                    try! realm.write {
                        ...
                        vc.passedOverStayObject = defaultOverStayObject
                        nav.pushViewController(vc, animated: true)
                    }
                }
            } else if passedReceiptObject?.category == "Overnight Stay" {
